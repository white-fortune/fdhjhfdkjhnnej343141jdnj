...
function PostSectionTool({ label, children, inAction }: { label: string, children: React.ReactElement<any>, inAction?: boolean }) {
	return (
		<div className={`tool gap-[2px] w-full flex items-center p-[5px] cursor-pointer rounded-[10px] ${inAction && 'hover:bg-gray-200'}`}>
...
function PostSectionTools({ extendedClass, inAction = false }: { extendedClass: string, inAction?: boolean }) {
	return (
		<div className={`tools ${extendedClass}`}>
			<div className="tools-left 
				gap-[15px] flex items-end
			">
				<PostSectionTool label='Media' inAction={inAction}>
					<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				...
				<PostSectionTool label='Poll' inAction={inAction}>
					<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				...
				<PostSectionTool label='Event' inAction={inAction}>
					<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function PostSection({ setOpen }: { setOpen: StateController<boolean>[1] }) {
	return (
...
function PostContainer({ setOpen }: { setOpen: StateController<boolean>[1] }) {
	const [text, setText] = useState<string>("")
	const [media, setMedia] = useState(false)

	const [activeIndex, setActiveIndex] = useState<number>(0)
	const [images, setImages] = useState<MediaPostImage[]>([
		{url: "https://placehold.co/600x400/orange/white", id: 1 },
		{url: "https://placehold.co/600x400/orange/white", id: 2 },
		{url: "https://placehold.co/600x400/orange/white", id: 3 },
		{url: "https://placehold.co/600x400/orange/white", id: 4 },
		{url: "https://placehold.co/600x400/orange/white", id: 5 }
	])
	

	function move(direction: Direction) {
		setActiveIndex(prev => {
			if (prev === images.length - 1 && direction === Direction.Next) {
				return 0
			} 
	...
	function removeImage(id: number, isLast: boolean) {
		if (isLast) move(Direction.Next)
	...
	return (
		<div className="post-container
			w-[70vh] min-[2000px]:w-[55vh]
			flex flex-col gap-1
			font-['Space_Grotesk'] bg-[var(--leftpanel-maincontainer-clr)] border rounded-[20px]
		">
			...
			">
				<div className="left-side w-[15%] h-[300px]">
					<div className="author-profile-pic h-[25%] min-[2000px]:h-[27%] flex justify-center items-center" onClick={() => console.log(text)}>
				...
				<div className="metadata w-[75%] flex flex-col min-h-[310px]">
					<div className="author-section h-[50px] flex items-center">
						<div className="author-fullname">
							<span className='text-base min-[2000px]:text-[18px]' onClick={() => setMedia(!media)}>Anirban Biswas</span>
						</div>
					</div>
					...
					<div className="post-data min-h-[250px] flex flex-col">
						...
						<div className="interactive-content mt-[10px] flex justify-center items-center">
							{ media && <PostContainer.MediaPost extendedClass='min-h-[300px] min-[2000px]:min-h-[360px]' images={images} actions={[removeImage, move]} activeIndex={activeIndex} />}
						</div>
					</div>
				</div>

				<div className="right-side w-[10%] max-h-[300px]">
					<div className="close-button h-[20%] min-[2000px]:h-[22%] flex justify-center items-center" onClick={() => setOpen(false)}>
			...
			<PostSectionTools extendedClass='p-5 w-full flex justify-between h-[60px]' inAction={true} />
		</div>
	)
}


PostContainer.MediaPostImage = function({ image, isActive, onClick }: { image: MediaPostImage, isActive: boolean, onClick: () => void }) {
	return (
		<div className={`img 
		...
		`} key={image.id}>
			<div className="cross-button absolute right-1 top-1" onClick={onClick}>
			...
			<img
				src={`${image.url}?text=${image.id}`}
				className="min-h-[260px] min-[2000px]:min-h-[300px] w-full rounded-md object-cover"
			/>
		</div>
	)
}
PostContainer.MediaPost = function({ extendedClass, images, activeIndex, actions: [removeImage, move] }: { extendedClass: string, images: MediaPostImage[], activeIndex: number, actions: [(id: number, isLast: boolean) => void, (direction: Direction) => void] }) {
	return (
		...
		`}>
			...
			">
				{ images.map((image, index) => {
					return <PostContainer.MediaPostImage image={image} isActive={index === activeIndex} onClick={() => removeImage(image.id, index === images.length - 1)} />
				})}
			</div>
			...
			">
				<div className="controller w-[40%] flex justify-between">
					<div className="left-controller flex justify-center items-center" onClick={() => move(Direction.Previous)}>
						<svg className='w-[40px] h-[40px] fill-none hover:fill-[rgba(0,0,0,0.2)]' viewBox="0 0 26 26" xmlns="http://www.w3.org/2000/svg">
					...
					<div className="number w-full flex justify-center items-center">
						<span className='count w-[40px] h-[40px] rounded-[50%] flex justify-center items-center bg-[#42ACDE]'>{ activeIndex + 1 }</span>
					</div>
...
function UILayer({ children }: { children: React.ReactNode[] }) {
	return (
...
function FloatingElements({ children }: { children: React.ReactNode | React.ReactNode[] }) {
	return (
		createPortal(
...
function App() {
	...
	const [openFloatingElement, setOpenFloatingElement] = useState<boolean>(false)
	...
	return (
		<UILayer>
				...
				<PrimaryMiddleSection>
					<HolderMiddleSection>
						...
						<SecondaryMiddleSection>
							<PostSection setOpen={setOpenPostContainer} />
			...
			{ openFloatingElement && <FloatingElements>
